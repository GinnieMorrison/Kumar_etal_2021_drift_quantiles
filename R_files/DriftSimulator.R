...
#
# The function will return a vector with one element
# for each simulation performed.
# Each element of the returned vector represents
# the final allele frequency for one simulation.

driftNeutral <- function(
  initial, total.pop, no.males = total.pop / 2, no.females = total.pop / 2,
  cycles = 30, plot = FALSE, sims = 1, sleep = 0
) {
  prog <- c(
      rep("A", initial * total.pop * 2), rep("B", (1 - initial) * total.pop * 2)
    )
  maleprog <- c()
  femaleprog <- c()
  frequency <- c()
  ...
  for (i in 1:cycles){
    #sample male alleles from total population
    maleprog <- sample(prog, size = no.males, replace = TRUE)
    maleprog <- rep(maleprog, length.out = total.pop)
    #sample females from total population
    femaleprog <- sample(prog, size = no.females, replace = FALSE)
  ...
  if (plot == TRUE)
    plot(
      dat, type = "l", ylim = c(0, 1), xlab = "Generation", ylab = "Frequency"
    )
  if (sims > 1) {
    for (j in 2:sims){
        Sys.sleep(sleep)
        prog <- c(rep("A", initial * total.pop * 2),
          rep("B", (1 - initial) * total.pop * 2))
        maleprog <- c()
        femaleprog <- c()
        frequency <- c()
        frequency[1] <- length(which(prog == "A")) / length(prog)
...
### Note: This function is free to use and distribute.
### Please provide appropriate credit when doing so.
