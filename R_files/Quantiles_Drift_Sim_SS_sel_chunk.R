...
##Simulate sampling 96 individuals with which to start the selection series (see paper for details). The sim frequencies generated
##will become the starting allele frequencies for the 15 generation drift simulation.
sample_96_gen0 <- sapply(fs, function(x) driftNeutral(x, total.pop=96, no.males=48, no.females=48, sims=sim, cycles=1))

##Simulate neutral drift for 15 generations
gen15_cycles<-sapply(sample_96_gen0, function(x) driftNeutral(x,total.pop=total,no.males=males,no.females=females,sims=1,cycles=num_gen))

##Sample the final generation as 96 inviduals sampled after a final generation.
final_sample <- sapply(gen15_cycles, function(x) driftNeutral(x, total.pop=96, no.males=48, no.females=48, sims=1, cycles=1))

##change list to a matrix to write. 
final_mat<-matrix(final_sample, nrow=rows, byrow=T)

filename<-sprintf('Drift_Sim_SS_%s.txt',chunk)
write.table(final_mat, filename,row.name=F,sep='\t')
